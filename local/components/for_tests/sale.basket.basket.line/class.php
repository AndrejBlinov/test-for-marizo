...
class SaleBasketLineComponent extends CBitrixComponent
{
	...
	private function setImgSrc(&$arBasketItems, $arElementId, $arSku2Parent)
	{
		...
		foreach ($arBasketItems as &$arItem)
		{
			...
			На примере компонента bitrix:sale.basket.basket.line показать, как можно кастомизировать компонент. 
			Например, фото товара брать не из стандартных полей, а из папки на сервере (/upload/photo/) 
			(название файла соответствует артикулу). 
			*/
			//получаем артикул товара если его нет
			
			if (!isset($arItem["ARTNUMBER"]))
			{
				$prop = CIBlockElement::GetByID($arItem["PRODUCT_ID"])->GetNextElement()->GetProperties();
				$imgNewPhoto = isset($prop["ARTNUMBER"]["VALUE"]) ? $prop["ARTNUMBER"]["VALUE"] : 'test' ;
			}
			else{
